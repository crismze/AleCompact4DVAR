...
!**************************STRET*******************************************
   ...
   real(8) :: pi,fourpi,r,x11,x,angle3,h,zero,r2,ym
   integer :: j1,j2,i5,i55,i6,jj,ixb1,ixa1
!**************************************************************************
!
   character(len=80) nom_file,nom_fichier
!
...
!
   ...
   integer :: jmax,jjmax,iimin,iimax
   integer :: ii,i21,i31,i7
   integer :: j3,j4,j5,jj1
   real(8) :: val,rlatp 
   real(8) :: epmach 
!
   nxyz=nx*ny*nz
...
!
   ...
   enddo
!
   open(30,file='number.txt',form='formatted')
     do nboucle=0266,2075
      call numcar(nboucle,suffixe)
      nom_file='cmlforcee'//suffixe
...
!
    call numcar(nboucle,suffixe)
    nom_file='vort'//suffixe
    longueur=index(nom_file,' ')-1
    open(20,file=nom_file(1:longueur)//'.dat',&
         form='formatted')
      do j=1,ny
      do i=1,nx
         write(20,*) (i-1)*dx,yp(j),tz(i,j,1)
      enddo
      write(20,*)
    ...
    write(30,*) suffixe
!
! creer le script gnuplot
    open(20,file='/media/disk/ndiaye/config1f2/lescript.gp')
    
        write(20,*) "set terminal jpeg"
        write(20,*) "set output '/media/disk/ndiaye/config1f2/vort"//suffixe//".jpeg'"
        write(20,*) "set palette color"
        write(20,*) "set title 'config1 forced at fundamental and first subharmonic'" 
        write(20,*) "set pm3d map" 
        write(20,*) "set cbrange[0:-1.2]"
        write(20,*) "set xrange[0:300]" 
        write(20,*) "set yrange[196:316]" 
        write(20,*) "set samples 1001" 
        write(20,*) "set isosamples 2" 
        write(20,*) "set size ratio -1" 
        write(20,*) "set palette rgbformulae 7,5,15 "
	write(20,*) "#--------------------------------#"
       
        write(20,*) "splot '/media/disk/ndiaye/config1f2/vort"//suffixe//".dat' u 1:2:3 notit "
        write(20,*) "set out"
        write(20,*) "set term x11"
        write(20,*) "!montage -geometry +4+4 profconfig1.jpeg vort"//suffixe//".jpeg bvort"//suffixe//".jpeg "
        write(20,*) "!gimp -i -b -c '(auto-crop bvort"//suffixe//".jpeg)' '(gimp-quit 0)' "
     close(20)
!
!
! lancer creation image *.jpeg   
!
    call system('/media/disk/ndiaye/config1f2/replace.sh')
    call system('gnuplot /media/disk/ndiaye/config1f2/'//'lescript.gp')
    write(*,*)'rm -f /media/disk/ndiaye/config1f2/vort'//suffixe//'.dat'
    call system('rm -f /media/disk/ndiaye/config1f2/vort'//suffixe//'.dat')
!
!
...
!  
   ...
   do i=iimin,i5
   cexx(i)=(i-1.)*dx 
   if (tan(angle3).eq.0.) stop 'division par zero '  
