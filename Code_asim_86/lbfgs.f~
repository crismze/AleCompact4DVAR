...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
      DOUBLE PRECISION GTOL,ONE,ZERO,GNORM,GNORM0,DDOT,STP1,FTOL,STPMIN,
...
C
  ...
  10  ITER= 0
ca
ca    Guarda norma del gradiente inicial para utilizar en TERMINATION TEST y Subrot. LB1
      GNORM0= DSQRT(DDOT(N,G,1,G,1))
ca
...
C
     ...
     *                     GNORM,GNORM0,N,M,X,F,G,STP,FINISH)
C
C    --------------------
...
C
      GNORM= DSQRT(DDOT(N,G,1,G,1))
ca      XNORM= DSQRT(DDOT(N,X,1,X,1))
ca      XNORM= DMAX1(1.0D0,XNORM)
ca      IF (GNORM/XNORM .LE. EPS) FINISH=.TRUE.
      IF (GNORM/GNORM0 .LE. EPS) FINISH=.TRUE.
ca
      IF(IPRINT(1).GE.0) CALL LB1(IPRINT,ITER,NFUN,
     *               GNORM,GNORM0,N,M,X,F,G,STP,FINISH)
      IF (FINISH) THEN
...
C
      ...
      DOUBLE PRECISION X(N),G(N),F,GNORM,GNORM0,STP,GTOL,STPMIN,STPMAX
      LOGICAL FINISH
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
...
C
 10   FORMAT('*************************************************')
 20   FORMAT('  N=',I8,'   NUMBER OF CORRECTIONS=',I2,
     .       /,  '       INITIAL VALUES')
...
C
      ...
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
ca      DATA MP,LP,GTOL,STPMIN,STPMAX/6,6,9.0D-01,1.0D-20,1.0D+20/
      DATA MP,LP,GTOL,STPMIN,STPMAX/6,6,0.5,1.0D-20,1.0D+20/
      END
C
C
C   ----------------------------------------------------------
