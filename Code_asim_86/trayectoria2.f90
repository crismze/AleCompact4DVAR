!********************************************************************
! Guarda errores para journal FDR-2014
PROGRAM trayectoria2
!
   USE paramx_m 
   USE paramy_m 
   USE paramz_m 
...
!    integer,parameter :: j_redg1=5,j_redg2=nyrg-4 ! bordes inferior y superior del campo asimilado en malla reducida grosera
   integer,parameter :: i_redg1=5,i_redg2=nxrg-4 ! bordes izquierdo y derecho del campo asimilado (REF) en malla reducida grosera
   integer,parameter :: j_redg1=5,j_redg2=nyrg-4 ! bordes inferior y superior del campo asimilado (REF) en malla reducida grosera
...
!
   ...
   real(8),dimension(fenetre,nxrg,nyrg,nz) :: ux_obsg,uy_obsg,wz_obsg,wz_obsg_int
   real(8),dimension(fenetre,nxrg,nyrg,nz) :: er_fin
   ...
   real(8),dimension(nxrg,nyrg,nz) :: ermt_fin   
...
!  ********************* Obs. *********************
   ...
   real(8) :: snorm4,error_ini,error_fin
!  ********************* LBFGS *********************
...
!  /home/ale/Programas/fiuba-2012/reduce_normaliza_suaviza_interpola_obs_asim82
   ncampo='cmcobsg'
   iob=iobs
   do f=1,fenetre
...
!  Abre archivos nuevos
   open(30,file='Error_ini',status='replace')
   open(40,file='Error_fin',status='replace')
!
...
!
!  Guarda estimacion del error corresp. a la trayectoria Inicial en dominio REF
   error_ini=snorm4(wz_obsg(1,:,:,:),wzitemp,nx,ny,nz,nxrg,nyrg,&
          jw,iw,w_xy,sumw_xy,n_nodos,n_nodos_max,&
          i_redg1,i_redg2,j_redg1,j_redg2,nxrg_asim,nyrg_asim)
   write(30,*) itime,error_ini
!
!  Guarda estimacion del error corresp. a la trayectoria Final en dominio REF
   error_fin=snorm4(wz_obsg(1,:,:,:),wzftemp,nx,ny,nz,nxrg,nyrg,&
          jw,iw,w_xy,sumw_xy,n_nodos,n_nodos_max,&
          i_redg1,i_redg2,j_redg1,j_redg2,nxrg_asim,nyrg_asim)
   write(40,*) itime,error_fin
!
!  Entrada: wz_obsg, wzftemp. Salida: er_fin
   call snorm4_campo (wz_obsg(1,:,:,:),wzftemp,er_fin(1,:,:,:),&
...
!
!     Guarda estimacion del error corresp. a la trayectoria Inicial en dominio REF
      error_ini=snorm4(wz_obsg(f+1,:,:,:),wzitemp,nx,ny,nz,nxrg,nyrg,&
             jw,iw,w_xy,sumw_xy,n_nodos,n_nodos_max,&
             i_redg1,i_redg2,j_redg1,j_redg2,nxrg_asim,nyrg_asim)
      write(30,*) itime,error_ini
!
!     Guarda estimacion del error corresp. a la trayectoria Final en dominio REF
      error_fin=snorm4(wz_obsg(f+1,:,:,:),wzftemp,nx,ny,nz,nxrg,nyrg,&
             jw,iw,w_xy,sumw_xy,n_nodos,n_nodos_max,&
             i_redg1,i_redg2,j_redg1,j_redg2,nxrg_asim,nyrg_asim)
      write(40,*) itime,error_fin
!
!     Entrada: wz_obsg, wzftemp. Salida: er_fin
      call snorm4_campo (wz_obsg(f+1,:,:,:),wzftemp,er_fin(f+1,:,:,:),&
             nx,ny,nz,nxrg,nyrg,jw,iw,w_xy,sumw_xy,n_nodos,n_nodos_max,&
             i_redg1,i_redg2,j_redg1,j_redg2,nxrg_asim,nyrg_asim)
!
   enddo ! do f=1,fenetrem
   close(30)
   close(40)
!
   ermt_fin=0.
!
!  Campo medio para la trayectoria de Error en dominio espacial reducido
   do f=1,fenetre ! dominio temporal de asimilacion
   do j=1,nyrg
   do i=1,nxrg
      ermt_fin(i,j,1)=ermt_fin(i,j,1)+er_fin(f,i,j,1)
   enddo
   enddo 
   enddo
!
!  Guarda campo medio para la trayectoria de Error en dominio de asimilacion
   open(10,file='ermt_fin',form='formatted')
   do j=j_redg1,j_redg2
   do i=i_redg1,i_redg2
      write(10,*) ermt_fin(i,j,1)/fenetre
   enddo
   enddo 
   close(10)
!
!
end PROGRAM trayectoria2
!
!********************************************************************
