...
!   promedio espacial de errorm
    errorvd(f) = SUM(errormd)/nxyrg
    errorv(f) = SUM(errorm)/nxyrg
  END DO
!
...
!
! *********************** error ***********************
  errord = error_obsd + error_iniud + error_inigd + error_tayd + error_gaud
  error = error_obs + error_iniu + error_inig + error_tay + error_gau
!
  RETURN
